...
export const createUser = async (req:Request<{},{},Userreq>,res:Response<Userres|Error>)=>{
        ...
        try{ 
            ...
            if(password) {
                const hashedPassword:string|void = await bcrypt.hash(password,10)
                const newUser:IUser = await User.create({name,email,age,password});
...
export const getUsers = async (req:Request,res:Response<Userres|Error>)=>{
...
export const getUserById = async (req:Request<{id:string}>,res:Response<Userres|Error>)=>{
...
export const updateUser = async(req:Request<{id:string},{},Userreq>,res:Response<Userres|Error>)=>{

    try{
        const id = req.params.id;
        const {name,email,age} = req.body
        const oldUser:IUser|null = await User.findById(id)
        
        if(oldUser){
            const newUser:IUser|null = await User.findByIdAndUpdate(id,{name,email,age},{new:true})
            if(newUser){
...
export const deleteUser = async (req: Request<{ id: string }>, res: Response<Error|Userres>) => {
    try {
  ...
  export const signUp = async (req:Request<{},{},Userreq>,res:Response<Userres|Error>)=>{
        ...
        try{
            ...
            try{    
                if(password){
                    ...
                    const newUser: IUser|null = await User.create({name,email,age,password:hashedPassword})
  ...
  export const login = async (req:Request<{},{},{password:string,email:string}>,res:Response<Userres|Error|UserResToken>)=>{
    ...
    try{
        ...
        const matchPassword:boolean|void = await bcrypt.compare(password,existingUser.password)
